    ...
    try:
        ...
        if "Topic" not in df.columns:
            ...
            def highlight_row(row):
                color = "#5B9279" if row["Topic"] in correct_topics else "#D95B5B"
                return [f"background-color: {color}; color: {'#FFFFFF' if is_dark_mode else '#000000'}"] * len(row)
            ...
            def to_excel_with_styles(dataframe, correct_topics):
                ...
                with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
                    ...
                    correct_format = workbook.add_format({'bg_color': '#5B9279', 'border': 1, 'font_color': '#FFFFFF'})
                    incorrect_format = workbook.add_format({'bg_color': '#D95B5B', 'border': 1, 'font_color': '#FFFFFF'})
